// frontend/src/pages/ManagerJobDetail.jsx
...
export default function ManagerJobDetail() {
  ...
  const btnPrimary = "btn btn-primary"; // keep existing button styles; these classes will pick project styles
  const btnGhost = theme === "dark" ? "btn btn-ghost text-slate-100 border-slate-600 hover:bg-slate-700/30" : "btn btn-ghost";

  // persist theme on document so descendant components / CSS vars work
  useEffect(() => {
  ...
  async function fetchJob() {
    ...
    try {
      ...
      if (e.status === 404) {
        // if no access or missing, go back
  ...
  return (
    <div className={`${pageBg} p-4 min-h-[60vh]`}>
      <div className="mb-4">
        <Link to="/manager/jobs" className={`text-sm ${linkColor} hover:underline`}>← Back to jobs</Link>
      </div>
      ...
      <div className={`${cardClass} p-4 rounded`}>
          ...
          <div>
            ...
            <div className={`mt-2 text-xs ${muted}`}>
              ...
              {job.assigned_technician?.user?.username && (
                <span className="ml-3">Assigned technician: <strong className={titleClass}>{job.assigned_technician.user.username}</strong></span>
              )}
            </div>
          </div>

          {/* Prediction card on the right (compact) */}
          <div>
        ...
        <div className="mt-4 flex gap-2">
          {/* Show Assign / Auto-assign only if job not already assigned */}
          {!job.assigned_technician ? (
            <>
              <button className={btnPrimary} onClick={() => setShowAssignModal(true)}>Assign</button>
              <AutoAssignButton
                jobId={job.id}
                onAssigned={async () => {
                  // refresh job after assignment done
            ...
            // If already assigned, show possibility to re-run auto-assign as "reassign" (optional)
            <div className={`text-sm ${muted}`}>Job is assigned — use the audit panel to view history.</div>
          )}
        </div>
      </div>
      ...
      {showAssignModal && (
        <AssignModal
          ...
          onAssigned={() => {
            ...
            // refetch job after assignment so page shows assigned_technician
            (async () => {
              try {
                const refreshed = await apiFetch(`/api/jobs/${id}/`, { token });
                setJob(refreshed);
              } catch (e) { console.error("refresh after assign failed", e); }
            })();
          }}
        />
      )}
